...
class GridZoom(ttk.Frame):
    ...
    def set_status(self):
        ...
        self.status.set(f"Position : x = {-self.x-1+int(self.width/2)} , y = {-int(self.height/2)+self.y+1} , cx = {self.cx:.2f} , cy = {self.cy:.2f} , Scale : {self.imscale:.2f}")
    ...
    def gridwheel(self, event):
        ...
        if event.num == 4 or event.delta == 120:  # scroll up
            sp = self.pixgrid
            self.pixgrid = int(sp*self.delta)
            if self.pixgrid > 400: self.pixgrid=400
    ...
    def show_background(self, event=None):
        ...
        h21 = (h2>0)*ds
        h22 = (h2>ds)*(h2-ds)
        data = ('{{{}{}{}{}}} '.format('gray50 ' * w11, 'white ' * w12, 'gray50 ' * w21, 'white ' * w22) * h11 +
                '{{{}}} '.format('white ' * sp) * h12 +
    ...
    def draw_reticle(self, event=None):
        ...
        h = self.height//2
        for i in range(4) :
            self.reticle[i] = self.canvas.create_line(w+3*(i==2)-10*(i==3),
                                                      h+3*(i==0)-10*(i==1),
                                                      w+11*(i==2)-2*(i==3),
                                                      h+11*(i==0)-2*(i==1),
                                                      fill="gray70",
